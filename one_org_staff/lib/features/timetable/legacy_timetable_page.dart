...
class _TimeTablePageState extends State<TimeTablePage> {
  ...
  List<_TimetableSection> _buildSections(List<TimetableLesson> lessons) {
    ...
    if (looseEntries.isNotEmpty) {
      ...
      sections.add(_TimetableSection(title: 'Other', lessons: looseEntries));
    }
  ...
  int _compareLessons(TimetableLesson left, TimetableLesson right) {
    final timeIdComparison = (left.timeId ?? 999).compareTo(
      right.timeId ?? 999,
    );
        ...
        gradient: LinearGradient(
          colors: [
            Color.alphaBlend(
              appColorsOf(
                context,
              ).accent.solid.withValues(alpha: isDarkMode ? 0.22 : 0.14),
...
class _TimetableLoadError extends StatelessWidget {
  const _TimetableLoadError({required this.message, required this.onRetry});
...
class _TimetableSection {
  const _TimetableSection({required this.title, required this.lessons});
