...
class StudentEntry {
  ...
  factory StudentEntry.fromJson(Map<String, dynamic> json) {
    ...
    final fullName = _firstString(json, const [
      'full_name',
      'fullName',
      'name',
    ]);
...
class GroupEntry {
  ...
  factory GroupEntry.fromJson(Map<String, dynamic> json) {
    ...
    final academicYearId =
        _asInt(json['academic_year_id'] ?? json['academicYearId']) ?? 0;
...
abstract class LessonPointsRepository {
  ...
  Future<void> createPointsBulk(String token, List<StudentPointDraft> points);
  ...
  Future<void> deleteContact(String token, {required int contactId});
}
