...
- Write operations require permissions: `lab_tests.create` / `lab_tests.update` / `lab_tests.delete` (via `@RequirePermissions`)
...
Returns `{ "ok": true, "message": "Lab test retrieved successfully", "result": { ... } }`
...
Returns `{ "ok": true, "message": "Lab test updated successfully", "result": { ... } }`
