...
- Write operations require permissions: `vital_signs.create` / `vital_signs.update` / `vital_signs.delete` (via `@RequirePermissions`)
...
Returns `{ "ok": true, "message": "Vital sign retrieved successfully", "result": { ... } }`
with the same `result` shape as the create response. `404` if not found.

## Example update

```http
PATCH /vital-signs/12
Authorization: Bearer <jwt>
Content-Type: application/json
```
```json
...
Returns `{ "ok": true, "message": "Vital sign updated successfully", "result": { ... } }`
with the updated fields applied. `400` if the body is empty; `404` if not found.

## Example delete

```http
DELETE /vital-signs/12
Authorization: Bearer <jwt>
```
